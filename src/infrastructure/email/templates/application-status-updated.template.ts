export const applicationStatusUpdatedTemplate = `<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    .dog__name { font-size: 19px; font-weight: 900; color: #18181b; margin: 0 0 4px 0; letter-spacing: -0.01em; }
    .dog__shelter { font-size: 13px; color: #71717a; font-weight: 600; }
    .badge { display: inline-block; padding: 3px 10px; border-radius: 999px; font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.06em; vertical-align: middle; margin-bottom: 6px; }
    .badge--review { background: #fef9c3; color: #854d0e; }
    .badge--approved { background: #dcfce7; color: #166534; }
    .badge--rejected { background: #fee2e2; color: #991b1b; }
    .message-box { border-radius: 14px; padding: 16px 18px; margin-bottom: 14px; }
    .message-box--review { background: #fffbeb; border: 1.5px solid #fde68a; }
    .message-box--approved { background: #f0fdf4; border: 1.5px solid #bbf7d0; }
    .message-box--rejected { background: #fff5f5; border: 1.5px solid #fecdd3; }
    .message-box__text { font-size: 14px; line-height: 1.65; margin: 0; color: #52525b; }
    .contact-box { background: #fafafa; border: 1.5px solid #f0f0f0; border-radius: 14px; padding: 14px 16px; margin-top: 4px; }
    .contact-box__label { font-size: 11px; font-weight: 700; color: #a1a1aa; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 6px; }
    .contact-box__value { font-size: 15px; font-weight: 700; color: #18181b; }
...
<body>
  <div class="container">
    ...
    <div class="intro">
      <p class="intro__hi">Hola {{applicantName}},</p>
      <h1 class="intro__title">{{title}}</h1>
      <p class="intro__desc">
        Hay una actualización sobre tu solicitud para adoptar a <span class="intro__strong">{{dogName}}</span>
        en <span class="intro__strong">{{shelterName}}</span>.
      </p>
    </div>
    ...
    <div class="message-box {{messageBoxClass}}">
      <p class="message-box__text">{{message}}</p>
    </div>

    {{#if showNote}}
    <div class="card">
      <h2 style="font-size:15px;font-weight:900;color:#18181b;margin:0 0 10px 0;letter-spacing:-0.01em;">Nota del refugio</h2>
      <p style="font-size:14px;color:#52525b;line-height:1.6;margin:0;">{{note}}</p>
    </div>
    {{/if}}

    <div class="card">
      <h2 style="font-size:15px;font-weight:900;color:#18181b;margin:0 0 10px 0;letter-spacing:-0.01em;">¿Tienes dudas?</h2>
      <p style="font-size:14px;color:#52525b;margin:0 0 12px 0;line-height:1.6;">
        Puedes contactar directamente a <strong>{{shelterName}}</strong> si tienes alguna pregunta o quieres más información.
      </p>
      <div class="contact-box">
        <div class="contact-box__label">Correo del refugio</div>
        <div class="contact-box__value"><a href="mailto:{{shelterEmail}}">{{shelterEmail}}</a></div>
      </div>
    </div>
